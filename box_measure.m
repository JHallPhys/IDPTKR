function SE=box_measure(SE,itt_box,Partition,bp,bq,Bpmesh,Bqmesh,Lpmesh,Lqmesh,Hus_Entropy,dgrid)
  L=length(Partition);
...
%              Pbox=Pbox; % THe probability inside the partition
             if Pbox<1e-12
                ...
                SE(itt_box,1)= SE(itt_box,1)+Pbox*log(Pbox); % This should be multiplied by a measure but is it dqdp or deps^2?
             end
