...
figure(1)
hold on
plot(1:1:N,abs(Es),'k.','Markersize',1)
figure(2)
hold on
plot(imag(log(Es)),real(log(Es)),'k.','Markersize',1)
...
Hus_av=zeros(N,N);
Hus_Entropy=zeros(N,N);
for t=1:n_efn
        disp([num2str(t),' out of ',num2str(n_efn)]) % keep track 
        Hus_av=Hus_av+abs(Hus(:,:,t)).^2;
        Pj=abs(Hus(:,:,t)).^2;
        Pj=Pj/N;
        Hus_Entropy=Pj.*log(Pj);
        fname=fname_husimi_single_efn(K_class,N,gamma,t,str_ext);
        parent_d = cd;  
...
subset_fname=fname_husimi(K_class,N,gamma,nset,str_ext);
parent_d = cd;  
cd './Husimi_dat' % Directory where matrix is stored
save(subset_fname,'Hus_Entropy'); % save it 
cd(parent_d)
